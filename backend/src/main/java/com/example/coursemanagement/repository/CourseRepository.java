...
public class CourseRepository {
    ...
    public int save(Course course) {
        // 使用正确的列名，基于实际数据库模式
        String sql = "INSERT INTO course (course_name, course_code, major_id, credit, total_hours, theory_hours, experiment_hours, design_hours, course_type_id, course_nature, course_category, exam_mark, program_id, teacher_ids) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public int update(Course course) {
        // 使用正确的列名，基于实际数据库模式
        String sql = "UPDATE course SET course_name = ?, course_code = ?, major_id = ?, credit = ?, total_hours = ?, theory_hours = ?, experiment_hours = ?, design_hours = ?, course_type_id = ?, course_nature = ?, course_category = ?, exam_mark = ?, program_id = ?, teacher_ids = ? WHERE course_id = ?";
